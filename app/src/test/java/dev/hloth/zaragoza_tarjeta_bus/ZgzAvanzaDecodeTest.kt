...
class ZgzTransportDecodeTest {
    @Test
    fun decodesTopUpCardType() {
        val block = hexToBytes("02699F000000000000000000000000F4")
        assertEquals(CardType.TOP_UP, ZgzTransport.decodeCardType(block))
    }
    ...
    fun decodesPersonalUnlimitedCardType() {
        ...
        assertEquals(CardType.PERSONAL_UNLIMITED, ZgzTransport.decodeCardType(block))
    }

    @Test
    fun rejectsCardTypeWithBadChecksum() {
        val block = hexToBytes("02699F00000000000000000000000000")
        assertThrows(TransportCardInvalidException::class.java) {
            ZgzTransport.decodeCardType(block)
        }
    }
    ...
    fun decodesDateFromSpecExample() {
        assertEquals(LocalDate.of(2026, 2, 14), ZgzTransport.decodeDate(0x34, 0x4E))
    }

    @Test
    fun decodesDateFrom2021Dump() {
        assertEquals(LocalDate.of(2021, 6, 28), ZgzTransport.decodeDate(0x2A.toByte(), 0xDC.toByte()))
    }

    @Test
    fun rejectsImpossibleDate() {
        assertThrows(TransportCardInvalidException::class.java) {
            ZgzTransport.decodeDate(0x00, 0x00)
        }
    }
    ...
    fun decodesRideFromSpecBlock5() {
        ...
        val tx = ZgzTransport.decodeTransaction(block)!!
    ...
    fun ignoresEmptyTransactionBlock() {
        assertNull(ZgzTransport.decodeTransaction(hexToBytes("00000000000000000000000000000000")))
    }

    @Test
    fun ignoresBlockWithInvalidTime() {
        val block = hexToBytes("020002260180CE16010D3454992D2400")
        assertNull(ZgzTransport.decodeTransaction(block))
    }
}
